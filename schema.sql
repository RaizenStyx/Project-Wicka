...
create table daily_draws (
  ...
  card_id bigint references tarot_cards not null,
  created_at timestamptz default now(),
  date_key text not null -- Stores "YYYY-MM-DD" to easily enforce one per day
);

-- Ensure a user can only have one row per date_key
create unique index on daily_draws (user_id, date_key);

alter table daily_draws enable row level security;

create policy "Users can view own draws" on daily_draws
  for select using (auth.uid() = user_id);

create policy "Users can insert own draws" on daily_draws
