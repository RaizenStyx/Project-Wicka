...
// import { X, Flame, BookOpen, History, Image as ImageIcon, Sparkles } from 'lucide-react'
// import Image from 'next/image'
// import { formatDistanceToNow, addHours } from 'date-fns'
// import { invokeDeity, banishDeity, extendInvocation } from '@/app/actions/deity-invocation-actions'
...
// export default function DeityModal({ isOpen, onClose, deity, isInvoked, isOwned, lastInvokedAt, lastOfferingAt }: Props) {
...
//     setMounted(true)
//     if (isOpen) {
//         setActiveTab(isInvoked ? 'ritual' : 'grimoire')
//         if (activeTab === 'ritual') fetchHistory()
//     }
//   }, [isOpen, isInvoked])
...
//       // 1. Tell the Widget (Client Side) to refresh data
//       if (typeof window !== 'undefined') {
//           window.dispatchEvent(new Event('deity-state-changed'))
//       }
//       // 2. Tell the Page (Server Components) to refresh data
//       router.refresh()
//   }
...
//     // Redirect to Altar view if not already there
...
//       await extendInvocation(deity.id)
//       notifyStateChange()
//   }
...
//             className="relative w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row h-[85vh] md:h-auto md:max-h-[90vh]"
//         >
            
//             {/* LEFT SIDE: VISUAL & ACTIONS */}
...
//                     {/* Mobile Close Button */}
//                     <button onClick={onClose} className="absolute top-2 right-2 p-2 bg-black/40 text-white rounded-full md:hidden backdrop-blur-sm z-10">
//                         <X size={18} />
//                     </button>

//                     <div className="absolute bottom-4 left-4 right-4 md:bottom-6 md:left-6 md:right-6">
//                         <h2 className="text-2xl md:text-3xl font-serif text-white drop-shadow-lg leading-none mb-1">{deity.name}</h2>
//                         <p className="text-purple-300 text-xs md:text-sm font-medium uppercase tracking-wider">{deity.pantheon}</p>
//                     </div>
//                 </div>
...
//                             disabled={loading}
//                             className="w-full py-2 md:py-3 rounded-lg bg-gradient-to-r from-purple-900 to-slate-900 border border-purple-500/50 text-purple-100 text-sm md:text-base font-medium hover:from-purple-800 hover:to-slate-800 transition-all flex items-center justify-center gap-2 shadow-[0_0_20px_-5px_rgba(168,85,247,0.5)]"
//                          >
//                             <Sparkles size={16} />
...
//                     <TabButton icon={<ImageIcon size={14} />} label="Gallery" active={activeTab === 'gallery'} onClick={() => setActiveTab('gallery')} disabled={!isOwned} locked={!isOwned} />
                    
//                     {/* Desktop Close Button */}
//                     <button onClick={onClose} className="hidden md:block ml-auto p-3 text-slate-500 hover:text-white transition-colors">
//                         <X size={20} />
//                     </button>
//                 </div>
...
//                             <div>
//                                 <h3 className="text-lg font-serif text-slate-200 mb-2">Domains</h3>
//                                 <div className="flex flex-wrap gap-2">
//                                     {deity.domain?.map((d: string) => (
//                                         <span key={d} className="px-3 py-1 rounded-full bg-slate-800 text-xs text-slate-300 border border-slate-700">{d}</span>
//                                     ))}
//                                 </div>
//                             </div>
//                             <div>
//                                 <h3 className="text-lg font-serif text-slate-200 mb-2">Lore & Description</h3>
//                                 <p className="text-slate-400 leading-relaxed text-sm">
//                                     {deity.description || "No description recorded in the archives."}
//                                 </p>
//                             </div>
//                             <div>
//                                 <h3 className="text-lg font-serif text-slate-200 mb-2">Symbols</h3>
//                                 <p className="text-slate-400 text-sm">{deity.symbols?.join(', ')}</p>
//                             </div>
//                         </div>
//                     )}
...
//                                     <h3 className="text-purple-200 font-serif mb-4 flex items-center gap-2">
...
//                                         <OfferingButton 
//                                             deityId={deity.id}
//                                             lastOfferingAt={lastOfferingAt}
//                                             onUpdate={notifyStateChange}
//                                         />
//                                         {/* BANISH BUTTON */}
//                                         <button onClick={handleBanish} className="px-4 py-2 rounded bg-slate-900 hover:bg-red-950/30 text-xs text-slate-400 hover:text-red-400 border border-slate-700 hover:border-red-900/50 transition-colors">
//                                             Banish
//                                         </button>
//                                     </div>
//                                 </div>
//                              )}
//                              <div>
//                                 <h3 className="text-lg font-serif text-slate-200 mb-4 flex items-center gap-2">
//                                     <History size={16} /> Past Invocations
//                                 </h3>
//                                 <div className="space-y-3">
//                                     {history.length === 0 ? (
//                                         <p className="text-slate-500 text-sm italic">No recorded rituals with this deity yet.</p>
//                                     ) : (
//                                         history.map((entry) => (
//                                             <div key={entry.id} className="flex items-center justify-between p-3 rounded bg-slate-950 border border-slate-800">
//                                                 <div className="flex flex-col">
//                                                     <span className="text-slate-300 text-sm font-medium">Invocation Rite</span>
//                                                     <span className="text-xs text-slate-500">{new Date(entry.started_at).toLocaleDateString()}</span>
//                                                 </div>
//                                                 <div className="text-right">
//                                                     <span className="text-xs text-slate-600">
//                                                         {entry.ended_at ? formatDistanceToNow(new Date(entry.started_at)) + ' ago' : 'Active Now'}
//                                                     </span>
//                                                 </div>
//                                             </div>
//                                         ))
...
//                                 Upload your own photo of your altar or statue for {deity.name}. 
...
//             disabled={disabled}
//             className={`
//                 flex items-center gap-2 px-3 md:px-4 py-3 text-sm font-medium border-b-2 transition-colors relative flex-1 md:flex-none justify-center
//                 ${active ? 'border-purple-500 text-purple-200' : 'border-transparent text-slate-500 hover:text-slate-300'}
//                 ${disabled ? 'opacity-50 cursor-not-allowed' : ''}
//             `}
//         >
//             {icon}
//             <span className="hidden md:inline">{label}</span>
//             <span className="md:hidden">{label === 'Grimoire' ? 'Lore' : label}</span>
...
import { createClient } from '@/app/utils/supabase/client'
import OfferingButton from './OfferingButton' // Import your new button
...
interface Props {
  ...
  isOwned: boolean 
  lastInvokedAt?: string | null
  lastOfferingAt?: string | null // NEW PROP
}
  ...
  return createPortal(
    <AnimatePresence>
      <div className="fixed inset-0 z-[9999] flex items-center justify-center p-2 md:p-4">
        ...
        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row h-[85vh] md:h-auto md:max-h-[90vh]">
                    ...
                    {activeTab === 'grimoire' && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 pb-6">
                            <div className="space-y-6">
                                <div><h3 className="text-lg font-serif text-slate-200 mb-2">Lore</h3><p className="text-slate-400 text-sm">{deity.description}</p></div>
                             ...
                             {isInvoked && (
                                <div className="p-4 rounded-xl bg-purple-950/20 border border-purple-500/30 mb-6">
                                    ...
                                    <div className="flex gap-3">
                                        ...
                                        {/* NEW: Using the OfferingButton Component */}
