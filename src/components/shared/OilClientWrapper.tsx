...
export default function OilClientWrapper({ oils, initialUserState }: Props) {
  ...
  const uniqueElements = Array.from(new Set(oils.map(c => c.element).filter(Boolean))).sort() as string[]
  ...
  const handleImageUpload = async (id: string, file: File) => {
    ...
    await saveUserOilImage(id, filePath)
  ...
  return (
    <>
      ...
      <GrimoireModal 
        ...
        // NEW: Pass the specific upload logic for Oils
